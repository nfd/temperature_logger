...
20120405 - wzdd ported to LUFA usbserial framework
**********************************************************************************/
...
void DS18X20_show_id_uart( uint8_t *id, size_t n )
{
	...
	for( i = 0; i < n; i++ ) {
		if ( i == 0 ) { fprintf(&USBSerialStream, "FC:" ); }
		else if ( i == n-1 ) { fprintf(&USBSerialStream, "CRC:" ); }
		if ( i == 1 ) { fprintf(&USBSerialStream, "SN: " ); }
		fprintf(&USBSerialStream, "%x ", id[i]);
		if ( i == 0 ) {
			if ( id[0] == DS18S20_FAMILY_CODE ) { fprintf(&USBSerialStream, "(18S)"); }
			else if ( id[0] == DS18B20_FAMILY_CODE ) { fprintf(&USBSerialStream, "(18B)"); }
			else if ( id[0] == DS1822_FAMILY_CODE ) { fprintf(&USBSerialStream, "(22)"); }
			else { fprintf(&USBSerialStream, "( ? )"); }
		}
	}
	if ( crc8( id, OW_ROMCODE_SIZE) )
		fprintf(&USBSerialStream,  " CRC FAIL " );
	else 
		fprintf(&USBSerialStream,  " CRC O.K. " );
}
...
static void show_sp_uart( uint8_t *sp, size_t n )
{
	...
	fprintf(&USBSerialStream, "SP:" );
	for( i = 0; i < n; i++ ) {
		if ( i == n-1 ) { fprintf(&USBSerialStream, "CRC: %x ", sp[i]); }
	}
}
	...
	const uint8_t cel, const uint8_t cel_frac_bits)
{
	...
	fprintf(&USBSerialStream, "%c%d.", subzero?'-':'+', (int)cel);
	itoa(cel_frac_bits*DS18X20_FRACCONV,buffer,10);
	for ( i = 0; i < 4-strlen(buffer); i++ ) {
		fputs("0", &USBSerialStream);
	}
	fprintf(&USBSerialStream, "%s°C", buffer);
}
...
#if DS18X20_VERBOSE
#define fprintf_verbose(...) fprintf(...)
#else 
...
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[])
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		fprintf_verbose(&USBSerialStream, "DS18X20_start_meas: Short Circuit!\r" );
	...
	uint8_t th, uint8_t tl, uint8_t conf)
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		fprintf_verbose(&USBSerialStream, "DS18X20_write_scratchpad: Short Circuit!\r" );
		ret = DS18X20_ERROR;
	}
...
uint8_t DS18X20_read_scratchpad( uint8_t id[], uint8_t sp[], uint8_t n )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		fprintf_verbose(&USBSerialStream, "DS18X20_read_scratchpad: Short Circuit!\r" );
		ret = DS18X20_ERROR;
	}
	...
	uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		fprintf_verbose(&USBSerialStream, "DS18X20_copy_scratchpad: Short Circuit!\r" );
		ret = DS18X20_START_FAIL;
	}
...
uint8_t DS18X20_eeprom_to_scratchpad( uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // only send if bus is "idle" = high
		...
		if ( retry_count ) {
			...
			fprintf_verbose(&USBSerialStream, "DS18X20_recall_E2: timeout!\r" );
			ret = DS18X20_ERROR;
		}
	} 
	else { 
		fprintf_verbose(&USBSerialStream, "DS18X20_recall_E2: Short circuit!\r" );
		ret = DS18X20_ERROR;
	}
