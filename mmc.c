...
#define CS_LOW()	while(0){} /* MMC CS = L */
#define	CS_HIGH()	while(0){} /* MMC CS = H */
...
#define	FCLK_SLOW()	SPCR &= 0xFB		/* Set slow clock (100k-400k) */
#define	FCLK_FAST()	SPCR &= 0xF8		/* Set fast clock (depends on the CSD) */
...
{
	return 1; // return (PORTE & 0x80) ? 0 : 1;
}
...
void power_on (void)
{
	...
	//DDRB  = 0b11000111;			/* 1 = output */

	// from ucontroller perspective:
	// /SS  : output (portb0)
	// mosi : output (portb2)
	// miso : input  (portb3)
	// sck  : output (portb1)
	// cs   : output (always connected to gnd)
	DDRB |= 0b00000111; // portb2, portb1, portb0: output
	DDRB &= 0b11110111; // portb3: input
